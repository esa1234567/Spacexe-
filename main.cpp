...
int Game_Mech(Level_Status gameset, ma_engine &engine) {
RESTART:
  pthread_t thread;
  pthread_t ster;
...
void GameMainPart() {
  ...
  ma_device device;
  result2 = ma_decoder_init_file("MenuMusic.mp3", NULL, &decoder);
...
REPRINTT:
  if (soundChk) {
    ...
    ma_engine_play_sound(&engine, "SelectOption.mp3", NULL);
    usleep(85000);
  ...
  for (;;) {
    ...
    if (input == '\n') {
      if (on == 1) {
        ma_engine_play_sound(&engine, "Enter.mp3", NULL);

        clear();
        Game_Mech(gameset, engine);
      }
      if (on == 4) {
      ...
      LEAVE:
        ...
        for (;;) {
          ...
          if (input4 == '\n') {
            clear();
            levelchk = 1;
            int exit_or_success = 0; // Game_Mech(gameset);
